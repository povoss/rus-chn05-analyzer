...
author: povoss
description: |
  慧龄云®中华05骨龄智能分析技能（RUS-CHN05计分法）。当用户上传手腕/手部X光片并请求骨龄分析、骨龄评估、生长发育评估、身高预测时触发。
  本技能对接慧龄云®骨龄人工智能检测系统API（host: www.pipitu.net），完成：注册激活→密码登录→图片上传→AI骨龄推算→生成完整中文诊断报告。
  支持两种调用路径：轻量路径（仅骨龄识别，/bmd/v2/cosBoneAgeOnLineByThirdPartner）和完整路径（骨龄+身高预测，/bmd/v2/predictHeightByBoneAge）。
  评估标准：中华05（RUS-CHN05计分法），由推理服务器端保证标准正确性。
  适用对象：3~18岁儿童青少年的家长、运动员管理者、体检机构等C端用户。
  关键词：骨龄、骨龄分析、骨龄评估、X光骨龄、生长发育、中华05、RUS-CHN05、手腕X光、发育评估、身高预测、性早熟筛查、骨龄推算、慧龄云
license: MIT-0
triggers:
  - 骨龄
  - 骨龄分析
